...
<div class="h-full w-full">
  ...
  <div class="h-[10%] flex justify-center items-center space-x-3">
    {#each diffComponents as [diffType]}
      <button
        data-diff-option={diffType}
        class={`btn focus:outline-none w-20 shadow rounded-full capitalize font-medium text-sm text-black ${
          $activeDiff === diffType ? 'bg-lightBlue-400' : 'bg-gray-200'
        }`}
        on:click={handleDiffOptionClick}
      >
        <div class="py-1 px-3 rounded-full focus:outline-none btn-content" tabindex={-1}>{diffType}</div>
      </button>
    {/each}
  </div>
</div>
