...
class WdioSnapshotTreeProvider implements TreeDataProvider<WdioSnapshot | WdioSpec> {
  ...
  getChildren(element?: WdioSpec): Thenable<WdioSnapshot[] | WdioSpec[]> {
    ...
    if (element) {
      return Promise.resolve(getAllSnapshots(element).sort((snapshotA, snapshotB) => {
        if (snapshotA.label < snapshotB.label) { return -1; }
        if (snapshotB.label < snapshotA.label) { return 1; }
        return 0;
      }));
    }
    
    const referencePath = path.join(this.workspaceRoot, testFolderPath, "__snapshots__", "reference");
    return Promise.resolve(getAllSpecs(referencePath).sort((specA, specB) => {
      if (specA.label < specB.label) { return -1; }
      if (specB.label < specA.label) { return 1; }
      return 0;
    }));
  }
}
